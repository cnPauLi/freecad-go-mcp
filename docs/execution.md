...
[English](execution.md) · [中文](execution.zh.md) · [Back to README](../README.md) · [Tools](tools.md) · [Configuration](configuration.md)
...
and timeout. Temporary scripts live in `~/.cache/freecad-mcp/headless` (the home
directory, because Flatpak sandboxes usually cannot see `/tmp`).

The executable is auto-detected in order: `freecadcmd`, `FreeCADCmd`,
`freecadcmd.exe`, and `freecad.cmd` on `PATH`, then the `org.freecad.FreeCAD`
Flatpak. Override it with:

```bash
freecad-go-mcp --freecadcmd "flatpak run --command=freecadcmd org.freecad.FreeCAD"
```

The `--freecadcmd` value is parsed with shell word-splitting rules, so quoted
arguments and escapes work; an unterminated quote fails at startup.
...
The client timeout covers both budgets plus a 30-second margin, with a 150-second
base value: it is the larger of 150 seconds and the per-operation budget above.
Other clients and MCP hosts must allow these response times in their own timeout
settings. Concurrent `execute_code` calls can be queued, but they still execute
...
Use `get_rpc_status` to identify the operation that is still running. The Go
client issues each call on its own HTTP connection and the RPC server handles
connections concurrently, so diagnostics return immediately while
`execute_code` is blocked. Document queries
